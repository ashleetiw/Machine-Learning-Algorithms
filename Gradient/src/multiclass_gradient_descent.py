...
class MultiClassGradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        self.uniq_labels=np.unique(targets)
        #  Creates an array to store the w's of all Gradient Descent learners
        classes=np.zeros((len(self.uniq_labels),features.shape[1]+1))
        i=0
        for c in self.uniq_labels:  
            # select one class set its label to 1 and all others to -1
            targets_c=targets.copy()
            targets_c[targets!=c]=-1
            targets_c[targets == c]=1
            learner.fit(features, targets_c) #, batch_size=None, max_iter=1000)
            # stores the w's 
            classes[i]=learner.weights
            i=i+1
    ...
    def predict(self, features):
        ...
        """
        # predict
        features1=features.copy()   # Do I need this?
        # append a column wirh ones in the features
        N_ones=np.ones((features.shape[0],1))
        features1=np.append(features1,N_ones,axis=1)

        # data_confidence is the distance (confidence) from each hyperline
        data_confidence=np.matmul(self.classes,features1.T)
        data_confidence=data_confidence.T
        # select the class with the max distance (confidence)
        y_label=data_confidence.argmax(axis=1)
       
        # assigns to each feature the correct class
        target_predict=self.uniq_labels[y_label]
