...
class MultiArmedBandit:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(steps):
            ...
            #choose action 
            choose = np.random.binomial(1,self.epsilon)
            ...
            rewards[int(np.floor(i/s))] += reward/s
            if(done):
              env.reset()
          
        state_action_values = np.array([list(Q) for i in range(env.observation_space.n)])
        # env.close()
