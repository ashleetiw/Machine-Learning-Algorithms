...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(steps):
            #choose action 
            choose = np.random.binomial(1,self._get_epsilon(float(i)/steps))
            p=np.random.rand(1)[0]
            if(p<self._get_epsilon(float(i)/steps)):
              choose=True
            else:
              choose=False
            ...
            N[state,action]+=1
            # best_next_action= np.argmax(Q[state_next])
            next_max = np.max(Q[state_next, :])

            Q[state,action] += (1.0/N[state, action])*(reward-Q[state, action] +self.discount*next_max)
            R[int(np.floor(i/s))] += reward/s
